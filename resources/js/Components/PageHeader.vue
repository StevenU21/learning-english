...
<template>
    ...
    <!-- Page header card -->
    <section class="relative overflow-hidden rounded-2xl bg-white/60 dark:bg-gray-900/60 backdrop-blur-xl border border-gray-200 dark:border-white/10 shadow-[0_8px_30px_rgb(0,0,0,0.12)] dark:shadow-[0_8px_30px_rgb(0,0,0,0.4)]">
            ...
            <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                <div>
                    <h1 class="text-2xl sm:text-3xl font-extrabold text-gray-900 dark:text-white tracking-tight flex items-center">
                        <i v-if="icon" :class="[icon, 'text-indigo-600 dark:text-white/90 mr-3']"></i>
                        {{ title }}
                    </h1>
                    <p v-if="subtitle" class="mt-1 text-gray-600 dark:text-white/80 text-sm">{{ subtitle }}</p>
                </div>
