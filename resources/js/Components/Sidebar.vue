<template>
  ...
  <aside :class="[
      ...
      effectiveCollapsed ? 'w-64 lg:w-16' : 'w-64 lg:w-64'
    ]">
    <div :class="['h-16 flex items-center border-b border-gray-200 dark:border-gray-700', effectiveCollapsed ? 'px-2 justify-start gap-2' : 'px-4 justify-between']">
      <Link :href="homeHref" class="flex items-center gap-2 overflow-hidden flex-shrink-0">
        <i class="fa-solid fa-layer-group w-6 text-center text-xl text-gray-800 dark:text-gray-100"></i>
        <h1 v-show="!effectiveCollapsed" class="text-xl font-bold text-gray-800 dark:text-gray-100 whitespace-nowrap">
          Learning <span class="text-[#FF2D20]">English</span>
        </h1>
      </Link>
      <div class="flex items-center gap-2">
        <!-- Desktop collapse toggle -->
        <button class="hidden lg:inline-flex" :class="[effectiveCollapsed ? 'p-1.5' : 'p-2', 'text-gray-500 hover:text-gray-700 dark:text-gray-400']" @click="toggleCollapsed" :title="collapsed ? 'Expandir' : 'Contraer'">
    ...
    <nav class="p-3 space-y-1 overflow-y-auto h-[calc(100vh-4rem)]">
      ...
      <template v-if="hasRole('admin')">
        <SidebarLink :href="route('units.index')" :active="route().current('units.index')" :compact="effectiveCollapsed" title-text="Unidades">
          <i class="fa-solid fa-layer-group w-5 text-center"></i>
          <span v-show="!effectiveCollapsed">Unidades</span>
        </SidebarLink>
        <SidebarLink :href="route('levels.index')" :active="route().current('levels.index')" :compact="effectiveCollapsed" title-text="Niveles">
          <i class="fa-solid fa-list w-5 text-center"></i>
          <span v-show="!effectiveCollapsed">Niveles</span>
        </SidebarLink>
        <SidebarLink :href="route('lessons.index')" :active="route().current('lessons.index')" :compact="effectiveCollapsed" title-text="Lecciones">
          <i class="fa-solid fa-book w-5 text-center"></i>
          <span v-show="!effectiveCollapsed">Lecciones</span>
        </SidebarLink>
        <SidebarLink :href="route('resources.index')" :active="route().current('resources.index')" :compact="effectiveCollapsed" title-text="Recursos">
          <i class="fa-solid fa-user w-5 text-center"></i>
          <span v-show="!effectiveCollapsed">Recursos</span>
        </SidebarLink>
        <SidebarLink :href="route('exercises.index')" :active="route().current('exercises.index')" :compact="effectiveCollapsed" title-text="Ejercicios">
          <i class="fa-solid fa-pencil w-5 text-center"></i>
          <span v-show="!effectiveCollapsed">Ejercicios</span>
        </SidebarLink>
        <SidebarLink :href="route('admin.progress.index')" :active="route().current('admin.progress.index')" :compact="effectiveCollapsed" title-text="Progreso">
      ...
      <template v-if="hasRole('student')">
        <SidebarLink :href="route('student.units.index')" :active="route().current('student.units.index')" :compact="effectiveCollapsed" title-text="Unidades">
          <i class="fa-solid fa-layer-group w-5 text-center"></i>
          <span v-show="!effectiveCollapsed">Unidades</span>
        </SidebarLink>
      </template>
      ...
      <SidebarLink :href="route('logout')" method="post" as="button" :compact="effectiveCollapsed" title-text="Cerrar sesión">
