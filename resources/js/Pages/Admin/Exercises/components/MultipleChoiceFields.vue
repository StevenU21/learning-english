  ...
  <div class="space-y-5">
    ...
    <div class="space-y-3">
      <div v-for="(opt, idx) in form.options" :key="idx" class="flex items-center gap-3">
        <input v-model="form.options[idx]" type="text" class="flex-1 rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300" placeholder="Opción" />
        <label class="flex items-center gap-1 text-xs text-gray-300">
          <input type="checkbox" :checked="isChecked(opt)" @change="toggleSolution(idx)" class="h-4 w-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500 dark:border-gray-600" />
          Correcta
        </label>
        <button type="button" @click="removeOption(idx)" class="text-xs text-red-400 hover:text-red-300">Quitar</button>
      </div>
    </div>
    <div class="flex gap-3">
      <button type="button" @click="addOption" :disabled="form.options.length >= 4" class="px-3 py-1.5 text-xs rounded-md bg-gray-200 text-gray-700 hover:bg-gray-300 disabled:opacity-50 dark:bg-gray-700 dark:text-gray-200 dark:hover:bg-gray-600">
