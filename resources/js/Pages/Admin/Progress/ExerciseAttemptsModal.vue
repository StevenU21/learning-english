...
<template>
  <GenericModal @close="close">
      ...
      <table class="w-full text-sm">
        ...
        <tbody>
          <tr v-for="a in attempts" :key="a.id" class="border-b border-gray-100 dark:border-gray-700/50">
            <td class="px-4 py-2 text-gray-800 dark:text-gray-200 truncate max-w-xs" :title="exercise?.prompt">
            ...
            <td class="px-4 py-2 text-gray-600 dark:text-gray-300">{{ a.answered_at ? new Date(a.answered_at).toLocaleString() : '-' }}</td>
          </tr>
        </tbody>
      </table>
    </div>
    ...
    <template #footer>
      <button @click="close" class="inline-flex items-center px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-medium rounded-md shadow">
        Cerrar
      </button>
    </template>
  </GenericModal>
