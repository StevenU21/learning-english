    ...
    <template #header>
      <div class="flex flex-col space-y-4">
        ...
        <div class="flex space-x-4">
          <div>
            <SelectInput v-model="form.unit_id" @change="applyFilters">
              <option value="">Todas las Unidades</option>
              <option v-for="unit in units" :key="unit.id" :value="unit.id">{{ unit.name }}</option>
            </SelectInput>
          </div>
          <div>
            <SelectInput v-model="form.user_id" @change="applyFilters">
              <option value="">Todos los Usuarios</option>
              <option v-for="user in users" :key="user.id" :value="user.id">{{ user.name }}</option>
            </SelectInput>
          </div>
          <div>
            <SelectInput v-model="form.lesson_id" @change="applyFilters">
              <option value="">Todas las Lecciones</option>
              <option v-for="lesson in lessons" :key="lesson.id" :value="lesson.id">{{ lesson.name }}</option>
            </SelectInput>
          </div>
          <div>
            <SelectInput v-model="form.status" @change="applyFilters">
          ...
          <div class="overflow-x-auto">
            <table class="w-full min-w-max">
              ...
              <tbody>
                ...
                <tr v-for="item in progressList" :key="item.id" class="transition-colors duration-150 hover:bg-gray-100 dark:hover:bg-gray-700">
                  <td class="text-gray-800 dark:text-gray-200 px-4 py-2">{{ item.user.name }}</td>
                  <td class="text-gray-600 dark:text-gray-400 px-4 py-2">{{ item.lesson.unit.name }}</td>
                  ...
                  <td class="text-gray-600 dark:text-gray-400 px-4 py-2 capitalize">{{ item.status }}</td>
                </tr>
              </tbody>
            </table>
          </div>
