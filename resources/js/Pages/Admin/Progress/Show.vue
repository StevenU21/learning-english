    ...
    <AuthenticatedLayout>
        ...
        <div class="py-0">
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-10">
                ...
                <div
                    ...
                    <div class="md:flex md:items-start md:space-x-8">
                        ...
                        <div class="w-full md:w-1/3 grid grid-cols-2 gap-4 text-sm text-gray-600 dark:text-gray-400">
                            <div class="flex items-center"><i class="fa-solid fa-user-tag mr-2"></i><span>{{
                                user.profile?.nickname || 'Sin datos' }}</span></div>
                            <div class="flex items-center"><i class="fa-solid fa-cake-candles mr-2"></i><span>{{
                                user.profile?.birthdate ? new Date(user.profile.birthdate).toLocaleDateString() :
                                    'Sin datos' }}</span></div>
                            <div class="flex items-center"><i class="fa-solid fa-graduation-cap mr-2"></i><span>{{
                                user.profile?.academic_level || 'Sin datos' }}</span></div>
                            <div class="flex items-center"><i class="fa-solid fa-venus-mars mr-2"></i><span>{{
                                user.profile?.gender || 'Sin datos' }}</span></div>
                        </div>
                    </div>
                </div>
                ...
                <ExerciseAttemptsModal v-if="showExerciseAttempts" :exercise="selectedExercise"
                    :attempts="selectedExerciseAttempts" @close="closeExerciseAttempts" />
            </div>
        </div>
    </AuthenticatedLayout>
</template>
