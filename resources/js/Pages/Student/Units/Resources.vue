  ...
  <AuthenticatedLayout>
    ...
    <template #header>
      <div class="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 flex items-center gap-2">
          ...
          Recursos de {{ unit.name }}
        </h2>
        <Link :href="route('student.units.index')"
              class="inline-flex items-center rounded-md bg-gray-800 px-3 py-2 text-xs font-semibold uppercase tracking-widest text-white hover:bg-gray-700 dark:bg-gray-200 dark:text-gray-800 dark:hover:bg-white">
    ...
    <div class="py-8">
      <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        <div v-if="!hasResources" class="text-center py-16 bg-white dark:bg-gray-800 rounded-xl border border-dashed border-gray-300 dark:border-gray-600">
        ...
        <div v-else class="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          <div v-for="r in resources" :key="r.id" class="group bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-5 flex flex-col">
            <div class="flex items-start justify-between gap-3 mb-3">
              <h3 class="font-semibold text-gray-800 dark:text-gray-200 text-sm leading-tight line-clamp-2">{{ r.name }}</h3>
            </div>
            <p class="text-xs text-gray-600 dark:text-gray-400 flex-1 leading-relaxed line-clamp-4 mb-4">{{ r.description || 'Sin descripción' }}</p>
